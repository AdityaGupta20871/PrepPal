        ...
        <div className='p-5 border rounded-lg my-10'>
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5'>
                {InterviewQuestion && InterviewQuestion.map((question,index)=>(
                    <h2 className={`p-2 bg-secondary rounded-full text-xs md:text-sm text-center cursor-pointer ${QuestionIndex == index && 'bg-blue-700 text-white'}`}>Question #{index+1}</h2>
                ))}
            </div>
